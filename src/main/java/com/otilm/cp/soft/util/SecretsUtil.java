 ...
 *
 * <p>
 * Values are self-describing: the first field of an encoded secret names its encoding, so
 * {@link #decodeAndDecryptSecretString(String)} can read whatever is stored without being told which scheme produced
 * it. New values are always written as {@link SecretEncodingVersion#V2 V2}; {@link SecretEncodingVersion#V1 V1} is
 * decrypted so values written before the upgrade stay readable.
 * </p>
 */
@Component
public class SecretsUtil {
    ...
    /**
     * Bounds on the iteration count read back from a stored value. The count travels with the secret so it can be
     * raised over time, which also means a tampered value could otherwise ask for an unbounded amount of work.
     */
    ...
    public void setEncryptionKey(@Value("${secrets.encryption.key}") String key) {
        ...
        if (PUBLISHED_DEFAULT_KEY.equals(key)) {
            logger
                    .warn("ENCRYPTION_KEY is not set, so secrets are protected with the default key "
     ...
     *
     * <p>
     * Deliberately not done when the key is set: migrations and tests build their own instances, and registering those
     * would let whichever was configured last decide the key the entity uses. Only a Spring-managed bean reaches this.
     * </p>
     */
    @PostConstruct
    ...
    public String encryptAndEncodeSecretString(String secret) {
        ...
        try {
            ...
            cipher.init(Cipher.ENCRYPT_MODE, deriveKey(salt, ITERATIONS), new GCMParameterSpec(GCM_TAG_BIT_LENGTH, iv));
            byte[] encrypted = cipher.doFinal(secret.getBytes(StandardCharsets.UTF_8));

            return SecretEncodingVersion.V2.getVersion() + "|" + Base64.getEncoder().encodeToString(encrypted) + "|"
    ...
    private String decryptV2(String secret) {
        ...
        try {
            Cipher cipher = Cipher.getInstance(AEAD_ALGORITHM);
            cipher.init(Cipher.DECRYPT_MODE, deriveKey(salt, iterations), new GCMParameterSpec(GCM_TAG_BIT_LENGTH, iv));
     ...
     *
     * <p>
     * The scheme is unauthenticated and its padding is why it was replaced. It is kept because the values are already
     * in deployed databases: without it they could not be read, and so could not be re-encrypted. Nothing writes this
     * encoding.
     * </p>
     */
    ...
    private String decryptV1(String secret) {
        ...
        try {
            ...
            SecretKeyFactory factory = SecretKeyFactory
                    .getInstance(LEGACY_ALGORITHM, BouncyCastleProvider.PROVIDER_NAME);
            cipher
                    .init(Cipher.DECRYPT_MODE,
                            factory.generateSecret(new PBEKeySpec(encryptionKey.toCharArray(), salt, iterations)));
            return new String(cipher.doFinal(encrypted), StandardCharsets.UTF_8);
        } catch (NoSuchAlgorithmException | NoSuchPaddingException | NoSuchProviderException | InvalidKeySpecException
