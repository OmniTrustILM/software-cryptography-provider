...
/**
 * Writes secrets the way the release before this one did, so the migration can be exercised against rows shaped like
 * the ones already in deployed databases.
 */
final class LegacyEncoding {
    ...
    static String v1(String secret, String encryptionKey) {
        ...
        try {
            ...
            return "v1|" + Base64.getEncoder().encodeToString(cipher.doFinal(secret.getBytes(StandardCharsets.UTF_8)))
    ...
    /**
     * The key the migration itself will use, so a value written here is one the migration is expected to be able to
     * read.
     */
