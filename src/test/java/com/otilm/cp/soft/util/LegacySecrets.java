 ...
 *
 * <p>
 * This is the encryption side of the old scheme, which {@link SecretsUtil} no longer has: it writes only the current
 * encoding, and keeps the old one for reading. Reproducing the old writer here is what lets the compatibility path be
 * tested against values shaped like the ones already sitting in deployed databases, rather than against something the
...
final class LegacySecrets {
    ...
    static String encryptV1(String secret, String encryptionKey) {
        ...
        try {
            ...
            return SecretEncodingVersion.V1.getVersion() + "|" + Base64.getEncoder().encodeToString(encrypted) + "|"
                    + Base64.getEncoder().encodeToString(salt) + "|" + ITERATIONS;
        } catch (Exception e) {
